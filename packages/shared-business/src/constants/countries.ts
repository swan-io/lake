...
type LazyValue<T> = ReturnType<typeof Lazy<T>>;
...
export const getCountryNameByCCA3 = (cca3: string) => {
  ...
  return countries.get().find(country => country.cca3 === cca3)?.name ?? cca3;
};
...
export const allCountriesItems: LazyValue<CountryItem<CountryCCA3>[]> = Lazy(() =>
  countries
    .get()
    .map(country => ({
...
export const individualCountriesItems: LazyValue<CountryItem<IndividualCountryCCA3>[]> = Lazy(() =>
  individualCountries
    .map(cca3 => ({
      name: names[cca3],
      cca3,
...
export const companyCountriesItems: LazyValue<CountryItem<CompanyCountryCCA3>[]> = Lazy(() =>
  companyCountries
    .map(cca3 => ({
      name: names[cca3],
      cca3,
...
export const isCountryCCA3 = (value: unknown): value is CountryCCA3 =>
  countries.get().some(({ cca3 }) => cca3 === value);

export const isCountryCCA2 = (value: unknown): value is CountryCCA2 =>
  countries.get().some(({ cca2 }) => cca2 === value);
...
const getBestNavigatorCountryIndividual = (): IndividualCountryCCA3 => {
  ...
  const country = individualCountriesItems.get().find(country => country.cca2 === cca2);
...
const getBestNavigatorCountryCompany = (): CompanyCountryCCA3 => {
  ...
  const cca2 = navigatorCountries.find(isCountryCCA2) ?? "FR";
  const country = companyCountriesItems.get().find(country => country.cca2 === cca2);
